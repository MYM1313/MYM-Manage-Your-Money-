...
const GetOutOfDebtIcon: FC = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-orange-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /><path strokeLinecap="round" strokeLinejoin="round" d="M16 8l-1.5 1.5M8 16l-1.5 1.5" /></svg>;
const GrowWealthIcon: FC = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-purple-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>;
...
const HeroSection: FC = () => {
    const { transactions, savingsGoals } = useContext(FinancialContext);
    ...
    return (
        <div className="space-y-4">
             <div className="animate-slide-up-fade-in text-left">
                ...
                <h1 className="text-xl font-bold font-montserrat text-gray-100 mt-1">Good morning, Aarav 👋</h1>
            </div>
...
const NetWorthPanel: FC = () => {
    const { investments, debts } = useContext(FinancialContext);

    const netWorth = useMemo(() => {
        const totalAssets = investments.reduce((sum, asset) => sum + asset.value, 0);
    ...
    const particles = useMemo(() => Array.from({length: 15}).map((_, i) => ({
        ...
        style: {
            ...
            '--spark-duration': `${Math.random() * 3 + 4}s`,
    ...
    return (
        <GlassmorphicPanel className="relative overflow-hidden text-center animate-slide-up-fade-in" style={{ animationDelay: '100ms' }}>
            {particles.map(p => <div key={p.id} style={p.style} className="absolute bg-teal-400/50 rounded-full animate-spark-float" />)}
            <p className="font-montserrat font-semibold text-gray-300">Total Net Worth</p>
            {investments.length === 0 && debts.length === 0 ? (
                ...
                <>
                    <div className="text-5xl font-bold font-montserrat text-gray-100 my-2 text-glow-blue relative">
                        <CountUp end={netWorth} />
                    </div>
                    <p className="text-sm text-gray-400">Update your assets to see monthly changes.</p>
                </>
            )}
        </GlassmorphicPanel>
    );
...
const CashFlowPanel: FC = () => {
    ...
    const cashFlow = useMemo(() => {
        return transactions.reduce((acc, t) => {
            ...
            return acc;
        }, { income: 0, expenses: 0, savings: 0, investment: 0 });
    }, [transactions]);
    
    const dataExists = transactions.length > 0;

    const CashFlowItem: FC<{ label: string; value: number; color: string; fullWidth?: boolean }> = ({ label, value, color, fullWidth = false }) => (
        <div className={`bg-black/20 p-4 rounded-2xl ${fullWidth ? 'w-full' : 'flex-1'}`}>
            <div className="flex items-center space-x-2 mb-1">
                 <div className={`w-2 h-2 rounded-full ${color}`}></div>
                 <p className="text-sm font-semibold text-gray-400">{label}</p>
            </div>
            <p className="font-montserrat font-bold text-2xl text-gray-100">
    ...
    return (
        <GlassmorphicPanel className="animate-slide-up-fade-in space-y-4" style={{ animationDelay: '200ms' }}>
             <h2 className="text-xl font-bold font-montserrat text-gray-100">Monthly Cash Flow</h2>
             {!dataExists ? (
                <div className="text-center py-8 text-gray-400">
                    <p>Add your transactions to see your cash flow.</p>
                </div>
             ) : (
                <>
                    <div className="flex gap-4">
                        <CashFlowItem label="Income" value={cashFlow.income} color="bg-green-400" />
                        <CashFlowItem label="Expenses" value={cashFlow.expenses} color="bg-rose-400" />
                    </div>
                    <div className="flex gap-4">
                        <CashFlowItem label="Savings" value={cashFlow.savings} color="bg-sky-400" />
                        <CashFlowItem label="Investment" value={cashFlow.investment} color="bg-purple-400" />
                    </div>
                </>
             )}
        </GlassmorphicPanel>
...
const AIActionCenter: FC<{ onNavigate: (view: string, params?: any) => void; onOpenAddFunds: (goalId: string) => void; }> = ({ onNavigate, onOpenAddFunds }) => {
    const { transactions, savingsGoals, debts } = useContext(FinancialContext);
    ...
    const insights = useMemo(() => {
        ...
        if (recentExpenses.length > 5) {
            ...
            // FIX: Replaced Object.entries with Object.keys for more robust type inference and to fix errors related to 'unknown' types in sort callback.
            ...
            if (sortedCategories.length > 0) {
                ...
                if (topCategoryAmount > 3000) {
                    generatedInsights.push({
                        ...
                        insightText: `This makes up over ${Math.round((topCategoryAmount / totalExpenses) * 100)}% of your expenses. Reviewing this can free up cash for your goals.`,
        ...
        if (underfundedGoal) {
             generatedInsights.push({
                ...
                insightText: "You're making progress! A small extra contribution now can significantly accelerate your timeline thanks to compounding.",
        ...
        // Insight 3: Start Emergency Fund if not present
        ...
        if (!emergencyFund) {
             generatedInsights.push({
                ...
                statusText: 'No Emergency Fund detected.',
                statusVisual: <MiniProgressBar value={0} target={1} colorClass="bg-teal-500" />,
                insightText: 'A safety net protects your investments and goals from unexpected life events. It is the foundation of a strong financial plan.',
        ...
        if(debts.some(d => d.apr > 15)) {
            generatedInsights.push({
                ...
                insightText: "High-interest debt actively works against your wealth-building efforts. Creating a payoff plan can save you thousands in interest.",
        ...
        if(generatedInsights.length === 0) {
             generatedInsights.push({
                ...
                statusText: 'Your finances look stable.',
                statusVisual: <MiniProgressBar value={1} target={1} colorClass="bg-lime-500" />,
                insightText: "Now is a great time to make your money work for you. Consistent investing, even small amounts, builds significant wealth over time.",
    ...
    }, [transactions, savingsGoals, debts, onNavigate, onOpenAddFunds]);
...
const FinancialRoadmapPanel: FC = () => {
    const { transactions } = useContext(FinancialContext);
    const roadmapSteps = [
    ...
    const MilestoneCard: FC<{ step: typeof roadmapSteps[0]; status: 'completed' | 'active' | 'locked' }> = ({ step, status }) => {
        ...
        return (
            <div className={cardClasses}>
                ...
                <button className={buttonClasses} disabled={isLocked}>
                    {isCompleted ? (
...
const FinancialOverviewCarousel: FC<{ onNavigate: (view: string, params?: any) => void }> = ({ onNavigate }) => {
    ...
    // FIX: Add local CircularProgress component definition to resolve 'not found' errors.
    const CircularProgress: FC<{ percentage: number; size?: number; strokeWidth?: number; colorGradientId: string; }> = ({ percentage, size = 80, strokeWidth = 8, colorGradientId }) => {
        ...
        return (
            <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="-rotate-90">
                ...
                <circle stroke={`url(#${colorGradientId})`} strokeWidth={strokeWidth} strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" fill="transparent" r={radius} cx={size/2} cy={size/2} style={{ transition: 'stroke-dashoffset 1.5s cubic-bezier(0.25, 1, 0.5, 1)' }} />
            </svg>
        );
    };
    ...
    const OverviewCard: FC<{ children: React.ReactNode; className?: string, onClick?: () => void }> = ({ children, className, onClick }) => (
        <div onClick={onClick} className={`flex-shrink-0 w-80 h-96 premium-glass !p-6 flex flex-col justify-between overflow-hidden relative transition-all duration-300 ${className}`}>
             {children}
        </div>
    );
        ...
        <div className="animate-slide-up-fade-in" style={{ animationDelay: '400ms' }}>
            ...
            <div className="flex space-x-4 overflow-x-auto no-scrollbar py-4 -mx-6 px-6">
                <svg width="0" height="0" className="absolute">
                    <defs>
                        <linearGradient id="savingsGradient" x1="0%" y1="0%" x2="0%" y2="100%"><stop offset="0%" stopColor="#38bdf8" /><stop offset="100%" stopColor="#34d399" /></linearGradient>
                        <linearGradient id="investGradient" x1="0%" y1="0%" x2="0%" y2="100%"><stop offset="0%" stopColor="#a78bfa" /><stop offset="100%" stopColor="#c084fc" /></linearGradient>
                    </defs>
                </svg>
                ...
                <OverviewCard onClick={() => onNavigate('Save')} className="border-sky-500/30 hover:shadow-[0_0_25px_rgba(56,189,248,0.2)]">
                    <div className="flex items-start justify-between">
                        <div>
                            ...
                            <p className="text-sm text-gray-400">{savingsData.goalCount} Goals</p>
                        </div>
                        <SavePurposeIcon />
                    </div>
                    <div className="flex flex-col items-center justify-center flex-1">
                        <div className="relative">
                            <CircularProgress percentage={savingsData.progress} colorGradientId="savingsGradient" size={120} strokeWidth={10}/>
                    ...
                    <div className="text-center">
                        ...
                        <p className="font-semibold text-lg text-gray-200">₹{savingsData.totalSaved.toLocaleString()}/<span className="text-sm text-gray-500">₹{savingsData.totalTarget.toLocaleString()}</span></p>
                    </div>
                </OverviewCard>
                ...
                <OverviewCard onClick={() => onNavigate('Invest')} className="border-purple-500/30 hover:shadow-[0_0_25px_rgba(168,85,247,0.2)]">
                    <div className="flex items-start justify-between">
                        <div>
                            ...
                            <p className="text-sm text-gray-400">{investmentData.goalCount} Goals</p>
                        </div>
                        <GrowWealthIcon />
                    </div>
                    <div className="flex flex-col items-center justify-center flex-1">
                         <div className="relative">
                            <CircularProgress percentage={investmentData.progress} colorGradientId="investGradient" size={120} strokeWidth={10}/>
                             <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <div className="text-center">
                         <p className="text-sm text-gray-400">Total Progress</p>
                        <p className="font-semibold text-lg text-gray-200">₹{investmentData.currentAmount.toLocaleString()}/<span className="text-sm text-gray-500">₹{investmentData.targetAmount.toLocaleString()}</span></p>
                    </div>
                </OverviewCard>
                ...
                <OverviewCard onClick={() => onNavigate('debt')} className="border-orange-500/30 hover:shadow-[0_0_25px_rgba(251,146,60,0.2)]">
                     <div className="flex items-start justify-between">
                        <div>
                            ...
                            <p className="text-sm text-gray-400">Your Freedom Plan</p>
                        </div>
                        <GetOutOfDebtIcon />
                    </div>
                    {debtData ? (
                ...
                <OverviewCard onClick={() => onNavigate('insurance')} className="border-teal-500/30 hover:shadow-[0_0_25px_rgba(45,212,191,0.2)]">
                     <div className="flex items-start justify-between">
                        <div>
                            ...
                            <p className="text-sm text-gray-400">Your Safety Net</p>
                        </div>
                        <ProtectEarnIcon />
                    </div>
...
const KeyFinancialMetricsPanel: FC = () => {
    ...
    const data = [
        { name: 'Income', value: metrics.income, color: '#4ade80' },
        { name: 'Expenses', value: metrics.expenses, color: '#f87171' },
        { name: 'Invest', value: metrics.investment, color: '#a855f7' },
        { name: 'Save', value: metrics.savings, color: '#38bdf8' },
        { name: 'Debt', value: metrics.debtTotal, color: '#f97316' },
        { name: 'Insurance', value: metrics.insuranceCoverage, color: '#2dd4bf' },
    ];
        ...
        if (active && payload && payload.length) {
            return (
                <div className="premium-glass !p-3 !rounded-xl !border-gray-600">
                    <p className="font-semibold text-gray-100" style={{ color: payload[0].fill }}>
