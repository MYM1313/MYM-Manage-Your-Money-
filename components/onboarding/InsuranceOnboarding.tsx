...
const PremiumToggle: FC<{ label: string; options: [string, string]; value: any; onSelect: (val: any) => void }> = ({ label, options, value, onSelect }) => (
    <div>
        ...
        <div className="flex gap-2 bg-black/30 p-1 rounded-xl">
            <button onClick={() => onSelect(options[0])} className={`flex-1 p-2 rounded-lg text-sm font-semibold transition-all ${value === options[0] ? 'bg-slate-600 text-white' : 'bg-transparent text-gray-300'}`}>{options[0]}</button>
            <button onClick={() => onSelect(options[1])} className={`flex-1 p-2 rounded-lg text-sm font-semibold transition-all ${value === options[1] ? 'bg-slate-600 text-white' : 'bg-transparent text-gray-300'}`}>{options[1]}</button>
        </div>
    </div>
);
                    ...
                    <OnboardingQuestion animationKey={step} direction={direction}>
                        ...
                        <div className="space-y-3 mt-4">
                            ...
                            <div>
                                ...
                                <div className="flex gap-2 bg-black/30 p-1 rounded-xl">
                                    <button onClick={() => updateData('hasMajorHealthConditions', true)} className={`flex-1 p-2 rounded-lg text-sm font-semibold transition-all ${insuranceOnboardingData.hasMajorHealthConditions === true ? 'bg-slate-600 text-white' : 'text-gray-300'}`}>Yes</button>
                                    <button onClick={() => updateData('hasMajorHealthConditions', false)} className={`flex-1 p-2 rounded-lg text-sm font-semibold transition-all ${insuranceOnboardingData.hasMajorHealthConditions === false ? 'bg-slate-600 text-white' : 'text-gray-300'}`}>No</button>
                                </div>
