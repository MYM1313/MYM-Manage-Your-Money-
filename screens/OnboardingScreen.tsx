...
const MultiSelect: FC<{ options: string[], selected: string[], onSelect: (val: string) => void, columns?: number }> = ({ options, selected, onSelect, columns = 2 }) => {
    ...
    return (
        <div className={`grid gap-2 ${columnClass}`}>
            {options.map(opt => (
                <button key={opt} onClick={() => onSelect(opt)} className={`p-3 text-sm rounded-lg font-semibold transition-all ${selected.includes(opt) ? 'bg-sky-500 text-white' : 'bg-white/10 text-gray-300'}`}>{opt}</button>
            ))}
        </div>
    );
};
...
const OnboardingScreen: FC<OnboardingScreenProps> = ({ onComplete, onSkip }) => {
    const [step, setStep] = useState(-1); // -1 welcome, 7 analyzing
    ...
    const handleNext = () => {
        ...
        let nextStep = step + 1;
        if (step === 1 && data.hasEmergencyFund === 'No') nextStep++; // Skip emergency fund amount
        if (step === 2 && data.invests === 'No') nextStep++; // Skip investment details
        if (step === 3 && data.hasDebts === 'No') nextStep++; // Skip debt details
        if (step === 4 && data.hasInsurance === 'No') nextStep++; // Skip insurance details
        setStep(nextStep);
    };
    ...
    const handleBack = () => {
        ...
        let prevStep = step - 1;
        if (step === 2 && data.hasEmergencyFund === 'No') prevStep--;
        if (step === 4 && data.invests === 'No') prevStep--;
        if (step === 5 && data.hasDebts === 'No') prevStep--;
        if (step === 6 && data.hasInsurance === 'No') prevStep--;
        setStep(prevStep);
    };
    ...
    const isNextDisabled = useMemo(() => {
        ...
        switch(step) {
            case 0: return !data.name || isNumInvalid(data.age) || isNumInvalid(data.monthlyIncome) || isNumInvalid(data.monthlyExpense);
            case 1: return data.hasEmergencyFund === null;
            case 2: return data.hasEmergencyFund === 'Yes' && isNumInvalid(data.emergencyFundAmount);
            case 3: return data.invests === null || (data.invests === 'Yes' && (isNumInvalid(data.totalInvestmentAmount) || data.investmentAssets.length === 0 || !data.riskLevel));
            case 4: return data.hasDebts === null || (data.hasDebts === 'Yes' && (data.debts.length === 0 || !data.debts[0].type || isNumInvalid(data.debts[0].outstandingAmount) || isNumInvalid(data.debts[0].interestRate) || (isNumInvalid(data.debts[0].emi) && isNumInvalid(data.debts[0].tenure)) ));
            case 5: return data.hasInsurance === null || (data.hasInsurance === 'Yes' && (data.insurancePolicies.length === 0 || !data.insurancePolicies[0].type || isNumInvalid(data.insurancePolicies[0].coverage) || isNumInvalid(data.insurancePolicies[0].premium) || !data.insurancePolicies[0].renewalDate));
            case 6: return false; // "Almost there" step
    ...
    if (step >= TOTAL_STEPS) return <AnalyzingAnimation />;

    return (
        <div className="h-full flex flex-col bg-gradient-to-b from-[#10141b] to-[#0D1117] text-gray-200 relative overflow-hidden p-6">
            <button onClick={onSkip} className="absolute top-6 right-6 z-20 px-4 py-2 bg-white/10 text-gray-300 text-sm font-semibold rounded-full hover:bg-white/20 transition-colors">Skip</button>
            ...
            <div className="flex-1 relative mt-12">
                {step === 0 && <OnboardingQuestion animationKey={step} direction={direction}>
                    <h2 className="text-2xl font-bold">Welcome! Let's personalize your journey.</h2>
                    <div className="space-y-4 mt-6">
                        <PremiumInput label="What’s your name?" value={data.name} onChange={e => updateData('name', e.target.value)} placeholder="e.g., Aarav" />
                        <PremiumInput label="How old are you?" type="number" value={data.age || ''} onChange={e => updateData('age', parseInt(e.target.value) || 0)} placeholder="e.g., 25" />
